...
class ObstacleTracker(Node):
    ...
    def _scan_cb(self, msg: LaserScan):
        ...
        n = min(len(msg.ranges), len(prev.ranges))
        ...
        for i in range(n):
            ...
            if not (msg.range_min <= r_now <= msg.range_max):
                continue
        ...
        for x, y in closing_pts:

            mx = tx + x * math.cos(yaw) - y * math.sin(yaw)
            my = ty + x * math.sin(yaw) + y * math.cos(yaw)
        ...
        for i in range(len(pts)):
            ...
            while queue:
                ...
                for j in range(len(pts)):
                    ...
                    if math.hypot(x2 - x1, y2 - y1) <= self._cluster_r:
                        visited[j] = True
                        queue.append(j)
